...
#--- appended by layout-fixer for test_vco -------------------------------
# Magic has no `device` line for an inductor in sky130A.tech, so a drawn
# spiral extracts as plain wire: both coil ports land on one node and Magic
# reports `Ports "B" and "A" are electrically shorted`. LVS cannot match a
# two-terminal inductor on topology, so the coil is black-boxed on BOTH
# sides. Same cell name on both sides, deliberately.
ignore class "-circuit1 vco_spiral_ind"
ignore class "-circuit2 vco_spiral_ind"
