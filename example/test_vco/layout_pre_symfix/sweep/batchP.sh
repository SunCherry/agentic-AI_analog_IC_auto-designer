...
runp() {
  ...
  echo "$TAG,$SEED,$WW,$WOV,$WD,$WA,$IT,$OV,$CL,$AS/$ASV,pa=$HP ctr=$CH,$BB,$WL,$SH,$FN,$FB" >> $CSV
  echo "$TAG seed=$SEED ww=$WW wd=$WD wa=$WA -> aspect=$AS/$ASV paHPWL=$HP ctrHPWL=$CH routed=$WL ov=$OV cl=$CL fb=$FB"
}
