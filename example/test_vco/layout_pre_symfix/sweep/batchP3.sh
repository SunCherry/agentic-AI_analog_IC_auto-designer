...
S=$L/sweep
CSV=$S/results.csv
runq() {  # tag seed ww wov wd wa iters thr t0
  TAG=$1; SEED=$2; WW=$3; WOV=$4; WD=$5; WA=$6; IT=$7; TH=$8; T0=$9
  python $L/port_aware_place.py $L/primitives/manifest.json \
    --iters $IT --seed $SEED --single-phase --no-render --accept-threshold $TH --t0 $T0 \
  ...
  CH=$(grep -m1 "stock centre-HPWL" $S/${TAG}_place.log | grep -oE "[0-9.]+ um" | tr -d ' um')
  ...
  echo "$TAG,$SEED,$WW,$WOV,$WD,$WA,$IT/thr$TH/t0$T0,$OV,$CL,$AS/$ASV,pa=$HP ctr=$CH,$BB,$WL,$SH,$FN,$FB" >> $CSV
  echo "$TAG s=$SEED wd=$WD wa=$WA it=$IT thr=$TH -> asp=$AS/$ASV pa=$HP routed=$WL ov=$OV cl=$CL fb=$FB"
}
