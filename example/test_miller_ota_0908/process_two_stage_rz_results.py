...
SUPPLIES = [("vdd", 1.8)]   # the deck drives ONE rail: `vdd VDD 0 dc=1.8`,
                            # and its `wrdata` line lists exactly `i(vdd)`.
...
if not os.path.isdir(_RAW_DIR):
    _RAW_DIR = _HERE
# FILLED IN BY schematic-agent #4c -- READ OFF THE DECK, NOT GUESSED.
# testbench/two_stage_rz_pre.spice writes:
#     write  ./simout_rz_pre.out v(vout)      <- AC raw, NOT "<design>_pre.out"
#     wrdata ./two_stage_rz_pre_op.txt i(vdd) <- op raw
AC_RAW = os.path.join(_RAW_DIR, "simout_rz_pre.out")      # deck's `write` target
OP_RAW = os.path.join(_RAW_DIR, "two_stage_rz_pre_op.txt")  # deck's `wrdata` target
